...
// CALIBRATION FIX
// Robot is physically bent 90 degrees at position 512.
// We subtract ~307 units (90 deg) to make the math match reality.
int ELBOW_OFFSET = -307; 

void setup() {
  Serial.begin(1000000); // Serial Monitor 1,000,000 baud
  delay(2000); 
...
void loop() {
  ...
  // Bottom Line (Move Reach OUT, keep Height same)
  for(float r=10; r<=15; r+=0.5) {
    moveToIK(r, 2);
    delay(50);
  }
  
  // Right Line (Move Height UP, keep Reach same)
  for(float h=2; h<=7; h+=0.5) {
    moveToIK(15, h);
    delay(50);
  }

  // Top Line (Move Reach IN, keep Height same)
  for(float r=15; r>=10; r-=0.5) {
    moveToIK(r, 7);
    delay(50);
  }

  // Left Line (Move Height DOWN, keep Reach same)
...
void moveToIK(float targetReach, float targetHeight) {
  ...
  // Convert Degrees to Servo Position (0-1023)
  // We apply ELBOW_OFFSET here to fix the hardware L-shape
